...
The client MUST only send the ADDR-REG-INFORM message for valid ({{!RFC4862}}) addresses of global scope ({{!RFC4007}}). This includes ULA addresses, which are defined in {{!RFC4193}} to have global scope. This also includes statically assigned addresses of global scope (such addresses are considered to have an infinite Valid Lifetime).
...
A function AddrRegRefreshInterval(address) is defined as min(MaxAddrRegRefreshInterval, 80% of the address's current Valid Lifetime). When calculating this value, the client applies a multiplier of AddrRegDesyncMultiplier to avoid synchronization causing a large number of registration messages from different clients at the same time. AddrRegDesyncMultiplier is a random value uniformly distributed between 0.9 and 1.1 (inclusive) and is chosen by the client when it starts the registration process, to ensure that refreshes for addresses with the same lifetime are coalesced (see below).

Whenever the client registers or refreshes an address, it calculates a NextAddrRegRefreshDeadline for that address as (now + AddrRegRefreshInterval) but does not schedule any refreshes.

Whenever the client receives a Prefix Information option (PIO, [RFC4861]) which changes the Valid Lifetime of an existing address by more than 1%, or when more than MaxAddrRegRefreshInterval has elapsed since the last refresh, then the client calculates a new AddrRegRefreshInterval. The client schedules a refresh for min(now + AddrRegRefreshInterval, NextAddrRegRefreshDeadline). If the refresh would be scheduled in the past, then the refresh occurs immediately.
...
MaxAddrRegRefreshInterval is chosen to ensure that address registration does not cause excessive multicast traffic, but still periodically notifies the network of which addresses are registered. The default value is 4 hours.

Justification: this algorithm ensures that refreshes are not sent too frequently, while ensuring that the server never believes that the address has expired when it has not. It also guarantees that refreshes will occur at most every MaxAddrRegRefreshInterval regardless of address lifetime. Specifically, after every registration:

- If the network never modifies the address lifetime (e.g., if no further PIOs are received, or if all PIO lifetimes decrease in step with the passage of time, or the address is statically configured), then refreshes occur every MaxAddrRegRefreshInterval.
- If the network modifies the address lifetime (i.e., changes the time at which the address will expire) the client ensures that a refresh is scheduled, so that server will be informed of the new expiry.
- Because AddrRegDesyncMultiplier is at most 1.1, the refresh never occurs later than a point 88% between the time when the address was registered or refreshed, and the time when the address will expire. This allows the client to retransmit the registration for up to 12% of the original interval before it expires. This may not be possible if the network sends a Router Advertisement (RA, [RFC4861]) that extends the address's lifetime very close to the time when the address would have expired. In this case, the client refreshes immediately, which is the best it can do.
